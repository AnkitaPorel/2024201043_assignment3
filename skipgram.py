...
def preprocess(sentence):
    """Preprocess a sentence: lowercase, remove punctuation, and stop words."""
...
def main():
    ...
    for sentence in processed_sentences:
        word_counts.update(sentence.split())
    # Create inverse mapping (ID → word)
    id_to_word = {idx: word for word, idx in vocab.items()}
    # Map frequencies to vocab indices
    ...
    try:
        skipgram_embeddings = torch.load('/content/skipgram.pt')
        wordsim_path = '/content/wordsim353crowd.csv'
        skipgram_correlation = evaluate_word_similarity(skipgram_embeddings, wordsim_path)
        print(f"Skip-gram Spearman Correlation: {skipgram_correlation}")
    except FileNotFoundError:
        print("Evaluation skipped: WordSim-353 file not found.")
